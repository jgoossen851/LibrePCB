...
void GerberGenerator::drawComponentOutline(
    ...
    const QString& value, const QString& manufacturer, const QString& mpn,
    const QString& footprintName, Function function) noexcept {
  if (path.getVertices().count() < 2) {
    qWarning() << "Invalid path was ignored in gerber output!";
    return;
  }
...
void GerberGenerator::flashComponentPin(
    ...
    const QString& value, const QString& manufacturer, const QString& mpn,
    const QString& footprintName, const QString& pin, const QString& signal,
